...
void AVLTreeVisualizer::animateRandom() {
    ...
    tree.~AVLTree();  // Call destructor to clean up old tree
    new (&tree) AVLTree();  // Reconstruct a new empty tree

    // Generate 15 random values using LoadRandomSequence
    int count = 15;
    int minValue = 1;    // Minimum value for nodes
    int maxValue = 99;   // Maximum value for nodes (keeping it 2 digits for display)

    // Generate random sequence
    int* randomValues = LoadRandomSequence(count, minValue, maxValue);
